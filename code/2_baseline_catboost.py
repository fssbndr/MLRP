...
model_output_path = os.path.join(
    args.output_dir, f"baseline_catboost{suffix}_model.cbm"
)
plot_output_path = os.path.join(
    args.plot_dir, f"baseline_catboost{suffix}_roc_curve.png"
)
...
plt.title("Receiver Operating Characteristic (ROC) Curve - CatBoost Test Set")
...
predictions_df = pl.DataFrame(
    {
        "model_name": "catboost.CatBoostClassifier",
        "model_args": f"baseline_catboost{suffix}",
        "num_shots": 0,
...
csv_output_path = os.path.join(
    args.output_dir, f"baseline_catboost{suffix}_results.csv"
)
