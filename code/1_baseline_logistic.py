...
from sklearn.metrics import roc_curve, auc  # Import sklearn metrics

# parse command line arguments
parser = argparse.ArgumentParser(description="Run baseline logistic regression.")
...
parser.add_argument("--plot", required=True, help="Path for the output ROC curve plot.")  # Add plot argument
...
X = X.fill_null(0)  # Fill numeric nulls first (might need adjustment if categorical nulls exist)

# Convert categorical columns to dummy variables
categorical_cols = ["Admission Type", "Admission Urgency"]
X = X.to_dummies(columns=categorical_cols, drop_first=True)  # drop_first avoids multicollinearity
...
output_dir_summary = os.path.dirname(args.output)
if output_dir_summary:
    os.makedirs(output_dir_summary, exist_ok=True)

# Write summary to file using args.output
